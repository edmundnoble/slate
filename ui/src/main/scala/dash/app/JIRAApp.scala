...
def issues(auth): httpPost("https://dashboarder.atlassian.net/rest/api/2/search/"; {};
                           { jql: .jql, maxResults: 10 }; auth + { ("Content-Type"): "application/json" }) | extractIssues;
...
let $auth as authHeaders in (httpGet("https://dashboarder.atlassian.net/rest/api/2/filter/favourite"; {}; {}; $auth) | extractFilters($auth) | contentFromFilter($auth))
"""

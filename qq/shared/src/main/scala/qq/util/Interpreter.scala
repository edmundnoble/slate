  ...
  def programInterpreterOf(source: String, program: InterpretedFilter): Interpreter = taskSwitch orElse
    Interpreter("program " + source + ", input:", {
      case input =>
        ...
        Right(
          ranProgram.flatMap(_.fold[Task[(String, Interpreter)]]({ errs => Task.raiseError(RuntimeError(errs)) }, { outputs =>
        ...
        Right(
          Runner.parseAndCompile(program).fold(
            err => Task.eval {
              ...
              result.flatMap(_.fold(es => Task.raiseError(RuntimeError(es)), Task.now)).map { outputs =>

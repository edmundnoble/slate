...
object QQASTTest extends utest.TestSuite {
  ...
  val tests = this {
    ...
    "optimize simple compositions" - {
      ...
      optimize(ComposeFilters(IdFilter, ComposeFilters(SelectKey("key"), IdFilter))).runAttempt.value ==> SelectKey("key")
    }

    "optimize pipes and dots to the same thing" - {
      optimize(QQParser.ensequencedFilters.parse(".key | .dang").get.value).runAttempt.value ==> ComposeFilters(SelectKey("key"), SelectKey("dang"))
      optimize(QQParser.ensequencedFilters.parse(".key.dang").get.value).runAttempt.value ==> ComposeFilters(SelectKey("key"), SelectKey("dang"))
    }

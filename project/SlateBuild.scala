...
object SlateBuild {
  ...
  val baseSettings: Seq[sbt.Def.Setting[_]] = Seq(
    ...
    scalacOptions in Compile ++= Seq("-Ywarn-unused-import", "-Ywarn-value-discard"),
  ...
  lazy val qq: CrossProject = crossProject.in(file("qq"))
    ...
    .settings(Dependencies.commonDeps: _*)
  ...
  lazy val uitests: Project = project.in(file("uitests"))
    ...
    .settings(baseSettings)
    .settings(Dependencies.commonDeps)
    .settings(dependOnChrome(ChromeBuild.unpackedProd, testOptions in Test))
